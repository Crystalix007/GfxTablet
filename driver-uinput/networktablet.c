...
void* send_current_screen(void* arg) {
	...
	surface = cairo_xlib_surface_create(disp, root, DefaultVisual(disp, scr), DisplayWidth(disp, scr),
...
int main(void) {
	...
	while (recvfrom(udp_socket, &ev_pkt, sizeof(event_packet), 0, (struct sockaddr*)&sock_t.from,
		...
		if (ev_pkt.version != PROTOCOL_VERSION) {
			fprintf(stderr,
			        "\nGfxTablet app speaks protocol version %i but driver speaks version %i, "
			        "please update\n",
