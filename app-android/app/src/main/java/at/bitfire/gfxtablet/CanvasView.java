      ...
      for (int ptr = 0; ptr < event.getPointerCount(); ptr++)
        if (!acceptStylusOnly || (event.getToolType(ptr) == MotionEvent.TOOL_TYPE_STYLUS)) {
          ...
          Log.v(
              TAG,
              String.format(
                  "Generic motion event logged: %f|%f, pressure %f",
                  event.getX(ptr), event.getY(ptr), event.getPressure(ptr)));
          switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_HOVER_EXIT:
              ...
              netClient
                  .getQueue()
                  .add(new NetEvent(Type.TYPE_BUTTON, nx, ny, npressure, -1, false));
              break;
          }
        }
      return true;
      ...
      for (int ptr = 0; ptr < event.getPointerCount(); ptr++)
        if (!acceptStylusOnly || (event.getToolType(ptr) == MotionEvent.TOOL_TYPE_STYLUS)) {
          ...
          Log.v(
              TAG,
              String.format(
                  "Touch event logged: action %d @ %f|%f (pressure %f)",
                  event.getActionMasked(),
                  event.getX(ptr),
                  event.getY(ptr),
                  event.getPressure(ptr)));
          switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_DOWN:
              if (inRangeStatus == inRangeStatus.OutOfRange) {
                ...
                netClient
                    .getQueue()
                    .add(new NetEvent(Type.TYPE_BUTTON, nx, ny, (short) 0, -1, true));
              }
            ...
            case MotionEvent.ACTION_CANCEL:
              ...
              if (inRangeStatus == inRangeStatus.FakeInRange) {
                ...
                netClient
                    .getQueue()
                    .add(new NetEvent(Type.TYPE_BUTTON, nx, ny, (short) 0, -1, false));
              }
              break;
          }
        }
